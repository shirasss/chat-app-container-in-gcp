...
artifact_registry_image=me-west1-docker.pkg.dev/grunitech-mid-project/shira-shani-chat-app-images/${appname}:${version}
gcloud compute ssh shira-shani-first-instance --project grunitech-mid-project --zone me-west1-a  --command "gcloud auth configure-docker me-west1-docker.pkg.dev; docker volume create chat-app-data; docker pull ${artifact_registry_image}; docker run -p 81:5000 ${artifact_registry_image}"
